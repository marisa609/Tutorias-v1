...
public class Sesion {
	...
	// las sesiones van de 20 en 20 minutos
	...
	// Get y Set
		...
		public Tutoria getTutoria() {
			return tutoria = new Tutoria (tutoria);
		}
		...
		private void setFecha(LocalDate fecha) {
			...
			if (fecha.isBefore(manana)) {
				throw new IllegalArgumentException("ERROR: Las sesiones de deben planificar para fechas futuras."); // Error en el test: de deben
			}
			this.fecha = fecha;
		}
		...
		private void setHoraFin(LocalTime horaFin) {
			...
			if (horaFin.isAfter(HORA_FIN_CLASES) || horaFin.isBefore(HORA_COMIENZO_CLASES)) {
				throw new IllegalArgumentException("ERROR: las hora para establecer la sesión no son válidas."); // ("ERROR: La hora de fin no es válida.");
			}
	...
	/*Para mostrar una sesión mostraremos los atributos de la tutoría
	 *  a la que pertenece y los suyos propios. 
	 */
	...
	//Método toString
		...
		public String toString() {
			return "tutoria=" + getTutoria().toString() + ", fecha=" + getFecha().format(FORMATO_FECHA) + ", horaInicio=" + getHoraInicio().format(FORMATO_HORA) + ", horaFin=" + getHoraFin().format(FORMATO_HORA) + ", minutosDuracion=" + getMinutosDuracion() +"";
		}
